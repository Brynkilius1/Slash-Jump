...
@onready var leader_board_button = $"../Menu/MainMenuButtons/LeaderBoardButton"
...
func _on_start_button_focus_entered():
	if start_button.global_position.y != 0:
		...
		selection_swords.position.x = 110
...
func _on_leader_board_button_focus_entered():
	TweenSelectionSwordPos(leader_board_button, 22)
	TweenBackroundCamera(114)
	audio_master.PlayRandomSound("MenuHover")
	selection_swords.position.x = 256
